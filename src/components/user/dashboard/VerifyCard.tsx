...
import QRScanner from "@/components/user/qr/QRScanner";
import { Html5Qrcode } from "html5-qrcode";
import { verifyWarranty } from "@/lib/api/user/warranty"; // ✅ Imported standard API method
...
export default function VerifyCard() {
  ...
  const handleVerify = async (idToVerify?: string) => {
    ...
    try {
      ...
      // ✅ Using your standardized API wrapper
      const data = await verifyWarranty(cleanId);
      ...
      setResult({
        valid: false,
        message: err.message || "Network error occurred during verification.",
      });
    } finally {
      setLoading(false);
    }
